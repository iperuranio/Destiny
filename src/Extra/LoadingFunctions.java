...
public class LoadingFunctions
{
	...
	public static void loadItems()
	{
		...
		for(String i : Main.getDestiny().getConfig().getStringList("list_items"))
		{
			HashMap<Integer, Integer> enchants = new HashMap<>();
			List<String> lore = new ArrayList<String>();
			ConfigurationSection cf = Main.getDestiny().getConfig().getConfigurationSection("items."+i);
	...
	public static void loadLuckyBlocks()
	{
		...
		{
			if(Main.getDestiny().getConfig().getConfigurationSection("lucky."+i) != null)
			{
				ConfigurationSection lucky = Main.getDestiny().getConfig().getConfigurationSection("lucky."+i);
	...
	public static void loadWetLuckyBlocks()
	{
		...
		for(int i = 1; i < Integer.MAX_VALUE; i++)
		{
			if(Main.getDestiny().getConfig().getConfigurationSection("wet_lucky."+i) != null)
			{
				ConfigurationSection lucky = Main.getDestiny().getConfig().getConfigurationSection("wet_lucky."+i);
				double l = lucky.getDouble("probability");
