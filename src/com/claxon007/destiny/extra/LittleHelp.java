...
import com.claxon007.destiny.messages.Errori;
...
public class LittleHelp {
	...
	public static void sendMessage(String message, String broadcast, Player p) {
		if (!message.equalsIgnoreCase("NULL")) {
			p.sendMessage(Destiny.LUCKYPREFIX + message);
		}

		if (!broadcast.equalsIgnoreCase("NULL")) {
			for (Player pl : Bukkit.getOnlinePlayers()) {
				pl.sendMessage(Destiny.LUCKYPREFIX + broadcast);
			}
		}
	}

	public static void executeCase(ExecutionType et, Player p, String PREFIX, Location l) {
		String message = Destiny.getDestiny().getConfig().getString(PREFIX + ".message").replace('&', '§')
				.replaceAll("%player%", p.getName());
		String broadcast = Destiny.getDestiny().getConfig().getString(PREFIX + ".broadcast").replace('&', '§')
				.replaceAll("%player%", p.getName());
		sendMessage(message, broadcast, p);

		if (et.equals(ExecutionType.COMMAND)) {
			String command = Destiny.getDestiny().getConfig().getString(PREFIX + ".command").toLowerCase()
					.replaceAll("%player%", p.getName());
			LittleHelp.command(command);
		} else if (et.equals(ExecutionType.ITEM)) {
			ItemStack daGivv = Objects.ITEMS.get(PREFIX);

			if (p.getInventory().firstEmpty() == -1) {
				p.sendMessage(Destiny.LUCKYPREFIX + "§cInventario pieno, item droppato per terra.");
			...
			Vault.addMoney(p, Destiny.getDestiny().getConfig().getDouble(PREFIX + ".amount"));
		} else {
